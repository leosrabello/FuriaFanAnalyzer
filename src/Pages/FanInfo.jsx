...
function FanInfo() {
  ...
  useEffect(() => {
    try {
      ...
      if (!user || user.type !== 'fan' || !storedFan) {
        navigate('/');
        return;
      }
      ...
      console.error('Erro ao carregar dados do fã:', error);
      navigate('/');
    } finally {
  ...
  const handleSave = () => {
    ...
    const updatedFanList = allFans.map(f => f.email === updatedFan.email ? updatedFan : f);
  ...
  return (
    ...
    >
      <div className="w-full flex justify-end p-4">
        <button onClick={handleLogout} className="bg-purple-600 hover:bg-purple-700 px-4 py-2 rounded transition-all">
          Sair
        </button>
      </div>

      <img src={furiaMascot} alt="Mascote FURIA" className="h-24 mt-2 mb-4 animate-bounce" />
      <h1 className="text-3xl text-purple-400 font-bold mb-4">Bem-vindo, {fanData.name}!</h1>
      ...
      {showMessage && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <div className="relative text-center text-white text-lg bg-zinc-900 p-6 rounded-lg shadow-lg max-w-lg w-full">
            ...
            <p>Você ganhou <span className="text-purple-500 font-semibold">{engagementScore}</span> pontos de engajamento!</p>
      ...
      <div className="bg-zinc-900 p-6 rounded-lg shadow-lg w-full max-w-md space-y-4 z-10">
        ...
        <input type="text" name="twitchUser" value={formData.twitchUser} onChange={handleChange} className="w-full p-2 rounded bg-zinc-700 border border-zinc-600" />

        <label>Twitter:</label>
        <input type="text" name="twitter" value={formData.twitter} onChange={handleChange} className="w-full p-2 rounded bg-zinc-700 border border-zinc-600" />

        <label>Instagram:</label>
        <input type="text" name="instagram" value={formData.instagram} onChange={handleChange} className="w-full p-2 rounded bg-zinc-700 border border-zinc-600" />

        <label>Cidade:</label>
        <input type="text" name="city" value={formData.city} onChange={handleChange} className="w-full p-2 rounded bg-zinc-700 border border-zinc-600" />

        <label>Tags (separadas por vírgula):</label>
        <input type="text" name="tags" value={formData.tags} onChange={handleChange} className="w-full p-2 rounded bg-zinc-700 border border-zinc-600" />

        <label className="flex items-center space-x-2 mt-2">
          <input type="checkbox" name="promoConsent" checked={formData.promoConsent} onChange={handleChange} />
          <span>Aceito receber emails promocionais da FURIA</span>
        </label>

        <label className="flex items-center space-x-2">
          <input type="checkbox" name="discordMember" checked={formData.discordMember} onChange={handleChange} />
          <span>Sou membro da comunidade da FURIA no Discord</span>
        </label>

        <button onClick={handleSave} className="w-full bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 px-4 rounded">
          Salvar Informações
        </button>
      </div>
